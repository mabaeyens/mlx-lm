...
class RotatingKVCache(_BaseCache):
    ...
    def to_quantized(self, group_size: int = 64, bits: int = 4) -> "RotatingQuantizedKVCache":
        if self.keep > 0:
...
class RotatingQuantizedKVCache(_BaseCache):
    ...
    def _update_in_place(self, keys, values):
        ...
        if self.keys is None or (prev >= cur_size and cur_size < self.max_size):
            ...
            new_k = _empty_quantized(
                B, n_kv_heads, new_size, k_head_dim, self.group_size, self.bits, keys.dtype
            )
            new_v = _empty_quantized(
                B, n_kv_heads, new_size, v_head_dim, self.group_size, self.bits, values.dtype
            )
            if self.keys is not None:
                self.keys = tree_map(
    ...
    @meta_state.setter
    def meta_state(self, v):
        self.keep, self.max_size, self.offset, self._idx, self.group_size, self.bits = map(
            int, v
        )
...
class BatchRotatingQuantizedKVCache(_BaseCache):
    ...
    def __init__(self, max_size, left_padding: List[int], group_size: int = 64, bits: int = 4):
        self.keys = None
        self.values = None
    ...
    def _temporal_order(self):
        ...
        if self.rotated:
            ...
            self.values = tree_map(lambda a: mx.roll(a, -self._idx, axis=2), self.values)
            self._idx = self.keys[0].shape[2]
            self.rotated = False
    ...
    def finalize(self):
        if self._lengths is not None:
            roll = mx.maximum(0, self.offset - self._lengths)
            self.keys = tree_map(lambda a: dynamic_roll(a, roll[:, None], axis=2), self.keys)
            self.values = tree_map(
                lambda a: dynamic_roll(a, roll[:, None], axis=2), self.values
            )
            self.left_padding += roll
    ...
    def extend(self, other):
        ...
        def pad(c):
            ...
            if c.keys is None:
                Bc = c.offset.shape[0]
                k = tree_map(
                    lambda a: mx.zeros((Bc, *a.shape[1:2], 0, a.shape[-1]), dtype=a.dtype), ref
                )
                v = tree_map(
                    lambda a: mx.zeros((Bc, *a.shape[1:2], 0, a.shape[-1]), dtype=a.dtype), ref_v
                )
            else:
                k, v = c.keys, c.values
        ...
        self.values = tree_map(lambda a, b: mx.concatenate([a, b], axis=0), pa[1], pb[1])
    ...
    def extract(self, idx):
        ...
        if self.rotated:
            ...
            cache.values = tree_map(lambda a: mx.roll(a, -self._idx, axis=2), cache.values)
            cache._idx = self.max_size
        cache.keys = tree_map(lambda a: mx.contiguous(a[:, :, padding : cache._idx]), cache.keys)
    ...
    def merge(cls, caches):
        ...
        if not all(
            (c.group_size, c.bits) == (caches[0].group_size, caches[0].bits) for c in caches
        ):
            raise ValueError(
                "BatchRotatingQuantizedKVCache can only merge caches with the "
        ...
        def alloc_like(ref_tuple):
            return tuple(
                mx.zeros((B, H, max_length, a.shape[-1]), dtype=a.dtype) for a in ref_tuple
            )
        ...
        cache = cls(
            caches[0].max_size, padding, group_size=caches[0].group_size, bits=caches[0].bits
        )
        cache.keys = keys
        cache.values = values
        cache.offset = mx.array(offsets)
